...
public class JhoveStatistics {
	...
	public static void JhoveOutputAnalysis(String jhoveFindings) throws IOException {
		...
		String outputCsv = name + "//" + "fixitTiffFindings" + ".csv";
		...
		csvsummary.println("Filename" + SEPARATOR + "JHOVE Findings" + SEPARATOR + "ErrorMessage");
		...
		for (int i = 0; i < len; i++) {
			if (lines.get(i).contains("Well-Formed and valid")) {
				...
				malformed++;
			} // else if ((lines.get(i).contains("error")) &&
				// (lines.get(i).contains("message"))) {
